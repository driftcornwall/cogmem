...
def _generate_hashes_from_db() -> dict:
    ...
    IMPORTANT: Only hash IMMUTABLE content fields (id, type, content, tags).
    Volatile fields (emotional_weight, recall_count, sessions_since_recall)
    change every session through decay and would cause false integrity mismatches.
    Must match incremental_merkle.py's _hash_memory() in the consolidation daemon.
    """
    ...
    file_hashes = {}
    for row in rows:
        # Hash only immutable content fields — matches daemon's incremental_merkle.py
