...
def consolidate_to_long_term():
    ...
    try:
        ...
        print("[AUTO-MEMORY] Cannot import memory_store — consolidation skipped", file=sys.stderr)
        return
    ...
    for item in ready:
        try:
            ...
            # Map salience to emotion value (higher salience = more emotional weight)
            emotion = min(max(item.get("salience", 0.5), 0.1), 0.9)

            memory_id, _ = store_memory(
                content=content,
                tags=tags,
                emotion=emotion,
            )
            ...
            print(f"[AUTO-MEMORY] Consolidation failed for item: {e}", file=sys.stderr)

    # Remove consolidated items from short-term buffer
    buffer["items"] = [i for i in buffer["items"] if not i.get("consolidated")]
    save_short_term(buffer)

    if consolidated_count > 0:
        print(f"[AUTO-MEMORY] Consolidated {consolidated_count} items to long-term memory",
              file=sys.stderr)
