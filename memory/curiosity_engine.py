...
5. Binding gap score  — memories recalled often but poorly integrated (low Phi)
...
# N5 v1.2: Added binding_gap — memories recalled but poorly integrated
ISOLATION_WEIGHT = 0.30      # Lonely nodes need connections most
BRIDGING_WEIGHT = 0.20       # Bridge candidates create structural value
DOMAIN_GAP_WEIGHT = 0.20     # Domain balance prevents cognitive blind spots
SURVIVOR_WEIGHT = 0.15       # Old survivors might be forgotten treasures
BINDING_GAP_WEIGHT = 0.15    # Recalled but poorly bound — need richer context
...
# --- N5 v1.2: Binding Gap Score ---

def score_binding_gap(memories: list[dict]) -> dict[str, float]:
    """
    Binding gap score: memories that HAVE been recalled but are poorly integrated.

    Different from isolation (which is about graph topology):
    - Isolation: never connected in co-occurrence graph
    - Binding gap: recalled but binding_strength is low (no affect/social/causal context)

    These memories benefit from re-exposure in richer contexts to build bindings.
    15/20 of our most-recalled memories had phi=0.000 — used as indices, never understood.
    """
    scores = {}

    # Only score memories with some recalls (otherwise they're isolation targets)
    recalled_mems = [m for m in memories if (m.get('recall_count') or 0) >= 2]
    if not recalled_mems:
        return {m['id']: 0.0 for m in memories}

    # Batch compute binding strengths via N5
    try:
        from binding_layer import bind_results, BINDING_ENABLED
        if not BINDING_ENABLED:
            return {m['id']: 0.0 for m in memories}

        fake_results = [{'id': m['id'], 'score': 0.5, 'preview': ''} for m in recalled_mems]
        bound = bind_results(fake_results)
        strength_map = {bm.id: bm.binding_strength for bm in bound}

        for mem in memories:
            mid = mem['id']
            recall_count = mem.get('recall_count') or 0
            if recall_count < 2:
                scores[mid] = 0.0
                continue
            bs = strength_map.get(mid, 0.0)
            # Low binding despite recalls = binding gap
            if bs < 0.15:
                scores[mid] = 1.0
            elif bs < 0.25:
                scores[mid] = 0.6
...
def compute_curiosity_scores(limit: int = CURIOSITY_TOP_N) -> list[dict]:
    ...
    bgap = score_binding_gap(memories)  # N5 v1.2
    ...
    for mem in memories:
        ...
        scores = {'isolation': iso, 'bridging': bri, 'domain_gap': dom,
                  'survivor': srv, 'binding_gap': bg}
        ...
        reasons = {
            ...
            'binding_gap': f'Recalled {mem.get("recall_count", 0)} times but poorly integrated (low Phi)',
        }
...
def main():
    ...
    if cmd == 'scan':
        ...
        for i, t in enumerate(targets, 1):
            ...
            print(f"   {t['reason']}")
            print(f"   iso={c['isolation']:.2f} bri={c['bridging']:.2f} dom={c['domain_gap']:.2f} srv={c['survivor']:.2f} bgap={c.get('binding_gap', 0):.2f}")
