...
def search_memories(query: str, limit: int = 5, threshold: float = 0.3,
    ...
    try:
        ...
        for result in results:
            ...
            if row:
                ...
                # Additive activation boost (preserves prior stage modifications; bias cap prevents explosion)
                result['score'] += 0.3 * activation
    ...
    if _expl and any(r.get('activation') for r in results):
        _expl.add_step('importance_freshness', imp_context, weight=0.3,
                       context=f'context={imp_context}, additive 0.3 * activation boost')
