...
BEGIN
    ...
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_memories_tier ON %I.memories(memory_tier)', agent_name, agent_name);
    ...
    EXECUTE format('
        CREATE TABLE IF NOT EXISTS %I.edges_v3 (
            ...
            thinking_about TEXT[],
            PRIMARY KEY (id1, id2),
            CHECK (id1 COLLATE "C" < id2 COLLATE "C")
        )', agent_name);
    ...
    -- --------------------------------------------------------
    -- 20. SESSION EVENTS (comprehensive event log)
    -- --------------------------------------------------------
    EXECUTE format('
        CREATE TABLE IF NOT EXISTS %I.session_events (
            ...
            source_block_type VARCHAR(20),
            tags TEXT[],
            extra JSONB DEFAULT ''{}''::jsonb
        )', agent_name, agent_name);

    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_events_session ON %I.session_events(session_id)', agent_name, agent_name);
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_events_time ON %I.session_events(event_time)', agent_name, agent_name);
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_events_type ON %I.session_events(event_type)', agent_name, agent_name);
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_events_platform ON %I.session_events(platform)', agent_name, agent_name);
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_events_entities ON %I.session_events USING GIN(entities)', agent_name, agent_name);
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_events_tags ON %I.session_events USING GIN(tags)', agent_name, agent_name);
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_events_fts ON %I.session_events USING GIN(to_tsvector(''english'', content))', agent_name, agent_name);
    EXECUTE format('CREATE INDEX IF NOT EXISTS idx_%s_events_time_type ON %I.session_events(event_time, event_type)', agent_name, agent_name);
...
CREATE TABLE IF NOT EXISTS shared.procedural_chunks (
    ...
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    PRIMARY KEY (chunk_id, agent)
);

CREATE INDEX IF NOT EXISTS idx_shared_chunks_agent
    ON shared.procedural_chunks(agent);
...
CREATE INDEX IF NOT EXISTS idx_swarm_msg_project ON shared.swarm_messages(project_id);
CREATE INDEX IF NOT EXISTS idx_swarm_msg_created_by ON shared.swarm_messages(created_by);
CREATE INDEX IF NOT EXISTS idx_swarm_msg_created_at ON shared.swarm_messages(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_swarm_msg_type ON shared.swarm_messages(project_id, message_type);
CREATE INDEX IF NOT EXISTS idx_swarm_msg_tags ON shared.swarm_messages USING GIN(tags);
CREATE INDEX IF NOT EXISTS idx_swarm_msg_fts ON shared.swarm_messages USING GIN(search_vector);
